...
public class BrowserPopSheet extends BottomSheetDialogFragment {
    Context context;
    boolean mLight;

    public static BrowserPopSheet newInstance() {
        return new BrowserPopSheet();
    }
    ...
    public void setupDialog(@NonNull Dialog dialog, int style) {
        ...
        refresh.setOnClickListener(view -> {
            ((BrowserPopup) BrowserPopup.getBrowser()).isRefresh();
            if (dialog.isShowing()) {
                dialog.dismiss();
            }
        });
        ...
        copy.setOnClickListener(view -> {
            ((BrowserPopup) BrowserPopup.getBrowser()).isCopy();
            if (dialog.isShowing()) {
                dialog.dismiss();
            }
        });
        ...
        open.setOnClickListener(view -> {
            ((BrowserPopup) BrowserPopup.getBrowser()).isOpen();
            if (dialog.isShowing()) {
                dialog.dismiss();
            }
        });
        ...
        smart.setOnClickListener(view -> {
            ((BrowserPopup) BrowserPopup.getBrowser()).isPins();
            if (dialog.isShowing()) {
                dialog.dismiss();
            }
        });
